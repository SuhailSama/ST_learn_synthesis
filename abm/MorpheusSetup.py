...
def main():
    ...
    model_file = 'morpheus_EP_2020_nov.xml'
...
def read_args():
    ...
    #print('args succesfully uploaded')
    return args


##############################################################################
def configure_XML_model(args, model_file):
    output_folder =r'sim%s'%(args.sim_id)
...
def change_XML(args,model_file):
    ...
    for var in variables:
        ...
        # Set cell ratios
        if symbol_name == 'simulation_time':
...
def run_model(model_file):
    #print('Modified simulation file succesfully')
    cmd = "singularity exec /usr/local/pace-apps/manual/packages/singularity_images/morpheus-2.2.0.sif morpheus -file %s > model_output.txt 2>&1" % model_file
    #print(cmd)
    status = subprocess.call(cmd, shell =True)
    os.system (cmd)
    print('Finished simulation successfully')
    return status
